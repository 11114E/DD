...
def compute_metrics(df, wquil_price):
    ...
    df.loc[:, 'Quil_Per_Hour'] = df['Quil_Per_Minute'] * 60
...
def update_balance():
    try:
        ...
        if not existing_df[(existing_df['Peer ID'] == peer_id) & (existing_df['Date'] == timestamp)].empty:
            return 'Duplicate data', 400  # Skip duplicates
...
def index():
    ...
    if data_frames:
        ...
        last_1440_quil_per_day = last_1440_quil_per_day.reindex(latest_balances.index).fillna(0)
        last_1440_quil_per_hour = last_1440_quil_per_hour.reindex(latest_balances.index).fillna(0)
